...
def passes_growth_filter(
    ...
    recent = valid[-(consecutive_periods + 1):]
    for i in range(1, len(recent)):
        ...
        if rev_growth is None or rev_growth < min_revenue_yoy_pct:
            return None
        if inc_growth is None or inc_growth < min_income_yoy_pct:
            return None
    ...
    latest = recent[-1]
    prev   = recent[-2]
    rev_g  = calc_yoy_growth(latest.net_sales, prev.net_sales)
    inc_g  = calc_yoy_growth(latest.net_income, prev.net_income)

    # フィルタを通過しているので None にはならないが、型安全のためデフォルト値を設定
    return FundamentalResult(
        code=valid[0].code,
        symbol=f"{valid[0].code}.T",
        name="",
        revenue_growth_pct=rev_g if rev_g is not None else 0.0,
        net_income_growth_pct=inc_g if inc_g is not None else 0.0,
    )
...
def filter_by_fundamentals(
    ...
    for code, stmts in statements.items():
        result = passes_growth_filter(
            stmts,
            min_revenue_yoy_pct=min_revenue_yoy_pct,
            min_income_yoy_pct=min_income_yoy_pct,
